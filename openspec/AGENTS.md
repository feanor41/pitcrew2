...
The control plane is shared memory, not an agent proxy. The host channel carries `user ↔ Daimon ↔ Aion ↔ specialists`; every specialist and Aion invoke `pitcrew` directly. Daimon interviews the user, clarifies intent and constraints, preserves conversational continuity, and forwards accepted requests. While Aion remains active, Daimon retains the current user-visible turn through host-native mailbox and user-steering waits, relays each meaningful Aion-acknowledged fact exactly once, sends one truthful notice no later than five minutes into each continuous quiet interval, and resumes waiting after forwarding steered input as requested state. It finalizes only after distinguishing completion, interruption, cancellation, host timeout, failure, blocker, clarification, user-owned gate, or abandonment. Once finalized, it never promises a future unsolicited update unless the selected host provides a real push channel.
...
Concurrent Daimon availability depends on host support for addressable agents and bounded native waits. Codex provides a bounded mailbox-and-steering wait only while the turn remains open. Pi provides `contact_supervisor` progress relays, but no stable native trace contract for the steered dual wait. OpenCode and Claude Code have no repository-verified bounded dual wait or unsolicited push. When required support is absent, Daimon discloses the limitation instead of simulating liveness. PitCrew adds no daemon, service, IPC, polling, durable inbox, database state, or lifecycle to guarantee it. A replacement Aion reconstructs context with `workflow show`.
