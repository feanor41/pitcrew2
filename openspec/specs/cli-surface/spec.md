...
`project inspect` SHALL resolve the canonical Git common directory without mutation and return `project_id`, `git_common_dir`, `checkout_root`, `initialized`, `repository_move_boundary`, central `paths` (`project_root`, `state_path`, `worktree_root`, `handle_root`), `legacy_acknowledged`, and `legacy` (`candidates`, `diagnostics`, `candidate_set_id`). For an initialized project, `legacy_acknowledged` SHALL be true only when the exact current `project_id` and `candidate_set_id` acknowledgement can be read safely. Inspection SHALL return `project consolidate` only for a current unacknowledged candidate set, use the normal workflow action after exact acknowledgement, and fail closed on an unsafe acknowledgement read. `project consolidate` SHALL accept a strict manifest with `project_id`, exact `candidate_ids`, zero or more whole-workflow `choices` containing `workflow_id` and `candidate_id`, and an optional `retain_existing` list of workflow IDs that explicitly keeps a divergent complete central graph. Retention instructions SHALL be unique, SHALL NOT overlap `choices`, and SHALL fail closed unless consumed by a real central-versus-legacy graph conflict. Unknown fields, malformed IDs, stale or incomplete source sets, incomplete graphs, unused retention instructions, and unchosen divergence SHALL fail closed. Success SHALL return the acknowledged `project_id` and `candidate_set_id` only after the atomic import commits.
