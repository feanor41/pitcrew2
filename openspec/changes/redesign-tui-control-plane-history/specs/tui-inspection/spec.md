...
(Previously: Activity recording was not explicitly prohibited for the TUI.)
...
(Previously: History lacked the created-time grid, workflow metadata, and actor-attributed activity timeline.)
...
(Previously: Branding required no size hierarchy, canonical version, distinct version color, or narrow-layout visibility.)
