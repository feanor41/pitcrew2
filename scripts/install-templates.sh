...
write_role() {
  ...
  handoff=$5
  {
    ...
    case $name in
      daimon)
        printf '%s\n' 'Do not invoke workflow commands. Forward accepted intent to Aion and report only Aion-acknowledged facts or clarification requests. Never infer progress.'
        ;;
      aion)
        printf '%s\n' 'PitCrew does not interpose on or prevent host filesystem writes. Treat the selected trace as the first admission gate: it must be acknowledged before any repository mutation; if not, stop before mutation and surface the capability boundary. Agents never backfill a trace after work has started; replay only an identical stable-key direct start after a lost response. Compose transcript-free handoffs from the workflow ID and current revision, role or unit ID, and opaque handle path; use the bounded coordination view. Aion alone owns routing, recovery, continuation, and completion. Never forge or bypass independent review, disclose handle contents, or mutate terminal workflows.'
        ;;
