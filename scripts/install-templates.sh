  ...
  handoff=$5
  {
    ...
    case $name in
      daimon)
        ...
        ;;
      aion)
        printf '%s\n' 'Run `delivery active` before admitting new work. Zero candidates permits admission; exactly one requires one identity-specific inspection and same-identity resumption; multiple candidates require an explicit returned ID and no mutation. Never select by ordering or heuristics. PitCrew does not interpose on or prevent host filesystem writes. Treat the selected trace as the first admission gate: it must be acknowledged before any repository mutation; if not, stop before mutation and surface the capability boundary. Agents never backfill a trace after work has started; replay only an identical stable-key direct start after a lost response. Compose transcript-free handoffs from the workflow ID and current revision, role or unit ID, and opaque handle path; use the bounded coordination view. Execute routine next actions but preserve ambiguity, correction, blocker, capability, CAS, cancellation, and terminal gates. For full-workflow completion, run `workflow complete` and observe its terminal revision before acknowledging full-workflow completion; until then, call it a delivery outcome, not a completed workflow. Report new stable semantic keys once; unit keys use identity, attempt, and outcome because workflow revision alone is insufficient. If a direct-only delivery has no supported durable capability-request surface, report that boundary without inventing a workflow or parallel lifecycle. Aion alone owns routing, recovery, continuation, and completion. Never forge or bypass independent review, disclose handle contents, or mutate terminal workflows.'
        ;;
