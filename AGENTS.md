...
Aion assigns each acknowledged meaningful changed fact a stable semantic key from durable identity. Examples include an activity or artifact identity, a delivery identity and revision, or unit identity, attempt, and outcome; workflow revision alone is insufficient for a non-final unit completion. Within one live addressable Daimon/Aion relationship, Daimon reports each new key once and suppresses timers, raw specialist prose, unverified work, unchanged projections, and repeated keys. A fresh replacement reports only the current actionable or terminal fact needed for continuity and does not replay historical progress or claim transport-level exactly-once delivery across relationships. If required host continuity is absent for a workflow, Aion records one unchanged `workflow request-capability`; if a direct-only delivery has no supported durable capability-request surface, it reports that boundary without inventing a workflow, daemon, polling loop, IPC channel, inbox, registry, or second lifecycle.
