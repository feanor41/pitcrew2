...
PitCrew is a local control plane for one person, one machine, and one project per invocation. Agents call the `pitcrew` subprocess directly; the control plane stores shared workflow state while the Master keeps only the workflow ID, current revision, goal, and short status lines.
...
4. Return a one-line completion status with the resulting revision and `next_action`. Do not relay artifact content through the Master.
...
- No embedded TUI, `internal/master`, `internal/installer`, or v1 migration.
...
|---|---|---|
| Master | `workflow new`, `workflow show`, `workflow approve-plan`, `workflow abandon`, optional `workflow complete` | Coordinate the user goal and approve execution. |
...
The role map is a prompt contract, not CLI authorization. `--actor` is declarative collision metadata, not authentication. The Implementer and Reviewer must use distinct actor labels for a unit revision.

## Hand-off rule

The Implementer returns only the opaque handle path to the Master. The Master passes only that path to the Reviewer. Handle contents never cross role boundaries.
