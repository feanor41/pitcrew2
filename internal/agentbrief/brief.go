...
func contracts() map[string]StableContract {
	...
	return map[string]StableContract{
		...
		"aion": c("aion", "Aion", "optional workflow context; no unit context", []string{
			"inspect active continuity first",
			"admit exactly once before mutation",
			"retain and resume one delivery identity on interruption or CAS",
			"contextually select the least-demanding sufficient route",
			"select direct_inline for safe well-understood coordinator work; delegated_direct when a bounded specialist handoff materially helps straightforward work; full_workflow when durable reasoning, evidence, or independent aggregate assurance is materially necessary",
			"file count is evidence, never a classifier; the same file count may select different routes when risk or uncertainty differs; a larger mechanical already-decided change may remain direct",
			"for stronger routing name the protected constraint and why the simpler route is materially insufficient",
			"delegate only the seven specialists",
			"never invent status or authority",
		}, aionHandoffs, []string{"delivery", "workflow"}),
