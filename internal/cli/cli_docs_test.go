...
func TestCLIReferenceExplainsControlPlaneCallStateFlow(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Daimon retains the active user-visible turn while Aion remains active",
		"host-native mailbox and user steering capabilities",
		"no later than five minutes into each continuous quiet interval",
		"interruption, cancellation, timeout, failure, blocker, clarification, user-owned gate, completion, or abandonment",
		"never promises a later unsolicited update after finalizing unless that host supplies a push channel",
		"missing bounded host liveness is disclosed rather than simulated",
		"Codex exposes a bounded mailbox wait that also accepts steered user input",
		"Pi exposes Aion-to-Daimon `contact_supervisor` progress relays",
		"OpenCode and Claude Code have no repository-verified bounded dual-wait or unsolicited push capability",
